...
int BPF_KPROBE(uprobe_malloc_entry, __u64 size)
{
    ...
    if (stack_len > 0) {
        ...
        bpf_map_update_elem(&pending_stacks, &tid, stack_val, BPF_ANY);
...
int BPF_KRETPROBE(uprobe_malloc_return, void *ret)
{
    ...
    struct stack_pcs_value *stack_val = bpf_map_lookup_elem(&pending_stacks, &tid);
    ...
    bpf_map_delete_elem(&pending_stacks, &tid);
